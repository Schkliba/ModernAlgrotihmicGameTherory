...
def test_week1():
    ...
    matrixdominated = np.array([[3, 2, 0], [4, 3, 2], [2, -1, 0]])

    row_strategy = np.array([[0.1, 0.2, 0.7]])
    column_strategy = np.array([[0.3, 0.2, 0.5]]).transpose()

    row_value = week1.evaluate_zero_sum(matrix=matrix, row_strategy=row_strategy, column_strategy=column_strategy)
    ...
    x = week1.best_response_calc_row(matrix=matrix, row_strategy=row_strategy)
    x2 = week1.strongly_dominated_row(matrixdominated)
    x3 = week1.strongly_dominated_column(matrixdominated)
    x4 = week1.pruning_dominated(matrixdominated)
    assert x == 1
    assert x2 == 0
    assert x3 == 0
    assert x4 == 2
